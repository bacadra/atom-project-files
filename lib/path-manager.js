...
export default class PathManager {
  ...
  constructor(S) {
    ...
    this.disposables.add(
      ...
      atom.config.onDidChange('fuzzy-finder.ignoredNames', () => { this.loadQ = true  }),

      atom.config.onDidChange('core.ignoredNames', () => { this.loadQ = true  }),

      atom.config.onDidChange('project-files.ignoredNames', () => { this.loadQ = true  }),
    )
  }
  ...
  updateEvent(events) {
    this.viewQ = false ; createdSkipQ = false
    for (e of events) {
      if (e.action==='created') {
