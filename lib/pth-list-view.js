...
export default class PthListView {
  ...
  async toggle () {
    if (this.panel && this.panel.isVisible()) {
      ...
      if (this.items) {
        ...
        this.selectListView.update({loadingMessage: 'Indexing project\u2026', infoMessage: null})
        setImmediate(()=>{
          this.loadList()
          this.selectListView.update({loadingMessage: null, infoMessage: 'Press `enter`, `alt-enter`, `ctrl-p`, `ctrl-a`, `ctrl-r` or `ctrl-n`'})
        })
      }
    }
  }
  ...
  get fuzz () {
    return atom.config.get('command-palette.useAlternateScoring') ? fuzzaldrinPlus : fuzzaldrin
  }
