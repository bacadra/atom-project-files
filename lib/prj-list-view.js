...
export default class PrjListView {
  ...
  constructor () {
    ...
    atom.commands.add(this.selectListView.element, {
      ...
      'prj-list:replace': (event) => {
        this.mode = 'replace'
        this.selectListView.confirmSelection();
        event.stopPropagation();
      },
      'prj-list:append': (event) => {
        this.mode = 'append'
        this.selectListView.confirmSelection();
        event.stopPropagation();
      },
    ...
    this.selectListView.element.classList.add('prj-list')
  }
  ...
  parseProjectFile() {
    pth = this.getProjectFilePath()

    if (!fs.existsSync(pth)) {
      return []
    }

    data = CSON.parseFile(pth)
