...
export default class PathListView {
  ...
  constructor(S) {
    ...
    this.disposables.add(
      ...
      atom.commands.add(this.selectListView.element, {
        'path-list:opin': () => this.didConfirmSelection('opin'),
        'path-list:opex': () => this.didConfirmSelection('opex'),
        'path-list:insp': () => this.didConfirmSelection('insp'),
        'path-list:insa': () => this.didConfirmSelection('insa'),
        'path-list:insr': () => this.didConfirmSelection('insr'),
        'path-list:insn': () => this.didConfirmSelection('insn'),
      }),
  ...
  update() {
    if (!this.S.pathManager.items) {
      ...
      this.S.pathManager.cache(() => {
        ...
        this.selectListView.update({items:this.S.pathManager.items, loadingMessage:null, infoMessage: 'Press Enter, Alt-Enter, Ctrl-P, Ctrl-A, Ctrl-R or Ctrl-N'})
      })
  ...
  didConfirmSelection(mode) {
    ...
    if (mode==='opin') {
      atom.workspace.open(path.join(item.pPath, item.fPath))
      ...
      if (!editor) {return}
      editor.insertText(item.fPath)

    } else if (mode==='insa') {
      editor = atom.workspace.getActiveTextEditor()
      if (!editor) {return}
      editor.insertText(path.join(item.pPath, item.fPath).replace(/\\/g, '/'))
      ...
      if (editorPath) {
        text = path.relative(path.join(editorPath, '..'), path.join(item.pPath, item.fPath)).replace(/\\/g, '/')
      } else {
        text = item.fPath
      }
      ...
      editor = atom.workspace.getActiveTextEditor()
      if (!editor) {return}
      editor.insertText(path.basename(item.fPath))
