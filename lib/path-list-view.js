...
export default class PathListView {
  ...
  constructor(S) {
    ...
    this.disposables.add(
      ...
      atom.commands.add(this.selectListView.element, {
        ...
        'path-list:update-query'   : () => this.updateQueryFromItem(),
      }),
  ...
  update() {
    if (!this.S.pathManager.items) {
      ...
      this.S.pathManager.cache(() => {
        ...
        this.selectListView.update({items:this.S.pathManager.items, loadingMessage:null, infoMessage: 'Press [ |Alt|Ctrl]-Enter, Alt+[Left|Right|Up|Down], Ctrl-[C|V] Ctrl-[P|A|R|N] or Alt-[0|/|\\]'})
      })
  ...
  didConfirmSelection(mode, params) {
    ...
    if (mode==='open') {
      ...
      if (params.rel==='p') {
        ...
        if (!editor) {
          atom.notifications.addError('Cannot insert path, because there is no activa text editor')
          return
        }
  ...
  updateQueryFromItem() {
    item = this.selectListView.getSelectedItem()
    this.selectListView.update({query:item.fPath})
  }
