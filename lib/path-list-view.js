...
export default class PathListView {
  ...
  constructor () {
    this.selectListView = new SelectListView({
      ...
      infoMessage: 'Press `enter`, `alt-enter`, `ctrl-v`, `ctrl-r ctrl-v`, `ctrl-a ctrl-v`, `ctrl-p ctrl-v` or `ctrl-n ctrl-v`',
      maxResults: 50,
      ...
      didConfirmSelection: (item) => {
        ...
        // open in atom
        } else if (this.insMode==4) {
          if (item.type==='Directory') {return}
          text = path.join(item.pPath, item.fPath)
          atom.workspace.open(text)

        // open external
        } else if (this.insMode==5) {
          text = path.join(item.pPath, item.fPath)
          shell.openPath(text)

        // insert only name
        } else if (this.insMode==6) {
          if (!editor) {return}
    ...
    atom.commands.add(this.selectListView.element, {
      'path-list:mode-1': (event) => {
        this.insMode = 1
        this.selectListView.confirmSelection();
        event.stopPropagation();
      },
      'path-list:mode-2': (event) => {
        this.insMode = 2
        this.selectListView.confirmSelection();
        event.stopPropagation();
      },
      'path-list:mode-3': (event) => {
        this.insMode = 3
        this.selectListView.confirmSelection();
        event.stopPropagation();
      },
      'path-list:mode-4': (event) => {
        this.insMode = 4
        this.selectListView.confirmSelection();
        event.stopPropagation();
      },
      'path-list:mode-5': (event) => {
        this.insMode = 5
        this.selectListView.confirmSelection();
        event.stopPropagation();
      },
      'path-list:mode-6': (event) => {
        this.insMode = 6
        this.selectListView.confirmSelection();
        event.stopPropagation();
      },
  ...
  show () {
    ...
    if (!this.items || this.reloadIgnore) { this.cacheAll() }
