...
export default class PathListView {
  ...
  update() {
    if (!this.S.pathManager.items) {
      ...
      this.S.pathManager.cache(() => {
        ...
        this.selectListView.update({items:this.S.pathManager.items, loadingMessage:null, infoMessage: 'Press Enter, Alt-Enter, Ctrl-P, Ctrl-A, Ctrl-R, Ctrl-N, Alt+Left, Alt+Right, Alt+Up or Alt+Down'})
      })
  ...
  didConfirmSelection(mode, side) {
    ...
    if (mode==='open-in') {
      ...
      atom.workspace.open(path.join(item.pPath, item.fPath), {split:side})
