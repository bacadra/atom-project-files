  ...
  constructor(S) {
    ...
    this.selectListView = new SelectListView({
      ...
      elementForItem: (item, {selected, index, visible}) => {
        ...
        matches = query.length>0 ? zadeh.match(Diacritics.clean(item.fPath), query) : []
        this.highlightMatchesInElement(item.fPath, matches, priBlock)
        if (this.useIcons && this.S.addIconToElement) {
          priBlock.classList.add('icon-line')
          this.disposIcons.add(
            this.S.addIconToElement(priBlock, path.join(item.pPath, item.fPath))
          )
        } else {
          priBlock.classList.add('icon-line', 'icon-file-text')
        }
      ...
      filter: (items, query) => {
        ...
        for (let item of items) {
          item.score = zadeh.score(Diacritics.clean(item.fPath), query)
    ...
    this.disposables.add(
      ...
      atom.config.observe('project-files.useIcons', (value) => {
        this.useIcons = value
      }),
    )
  ...
  update() {
    if (this.S.pathManager.loadQ) {
      ...
      this.S.pathManager.cache(() => {
        ...
        infoMessage = this.showKeystrokes ? ['Press ', <span class='keystroke'>Enter</span>, ', ', <span class='keystroke'>Alt|Ctrl-Enter</span>, ', ', <span class='keystroke'>Alt-Left|Right|Up|Down</span>, ', ', <span class='keystroke'>Alt-C|V Alt-P|A|R|N</span>, ' or ', <span class='keystroke'>Alt-0|/|\|Q|S</span>] : null
        this.selectListView.update({items:this.S.pathManager.items, loadingMessage:null, infoMessage:infoMessage})
      })
