...
export default class ProjectListView {
  ...
  constructor(S) {
    ...
    this.disposables.add(
      ...
      new File(this.getProjectFilePath()).onDidChange(() => { console.log('project-null') ; this.items = null }),
    )
  }
  ...
  update() {
    if (!this.items) {
      ...
      this.cache(() => {
        this.selectListView.update({items:this.items, loadingMessage:null, infoMessage: 'Press Enter, Alt-Enter or Shift-Enter'})
      })
    }
  }
  ...
  cache(callback) {
    ...
    for (item of this.items) {
      ...
      if (item.subsQ) {
        for (pPath of item.paths) {
          promises.push(this.globPromise(pPath))
        }
      }
    }
