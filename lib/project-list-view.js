  ...
  constructor(S) {
    ...
    this.selectListView = new SelectListView({
      ...
      elementForItem: (item) => {
        ...
        if (item.group) {
          groupBlock = document.createElement('span')
          groupBlock.classList.add('group')
          groupBlock.textContent = item.group
          priBlock.appendChild(groupBlock)
          spaceBlock = document.createElement('span')
          spaceBlock.textContent = ': '
          priBlock.appendChild(spaceBlock)
        }
        this.highlightMatchesInElement(item.title, this.selectListView.getQuery(), priBlock)
        priBlock.classList.add('icon')
        priBlock.classList.add('icon-file-directory')
        li.appendChild(priBlock)

        secBlock = document.createElement('div')
        secBlock.classList.add('secondary-line')
        for (fPath of item.paths) {
          pathBlock = document.createElement('div')
          pathBlock.textContent = fPath
      ...
      filter: (items, query) => {
        ...
        for (let item of items) {
          item.score = this.fuzz.score(item.title, query)
